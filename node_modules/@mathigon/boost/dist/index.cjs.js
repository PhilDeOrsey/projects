...
__export(index_exports, {
  ...
  SVGBaseView: () => SVGBaseView,
  SVGParentView: () => SVGParentView2,
  WindowView: () => WindowView,
  animate: () => animate,
  batch: () => batch,
  bindAccessibilityEvents: () => bindAccessibilityEvents,
  bindEvent: () => bindEvent,
  canvasPointerPosition: () => canvasPointerPosition,
  cleanSVG: () => cleanSVG,
  compile: () => compile,
  compileString: () => compileString,
  confetti: () => confetti,
  copySVGStyles: () => copySVGStyles,
  deferredPost: () => deferredPost,
  ease: () => ease,
  enter: () => enter,
  exit: () => exit,
  fromQueryString: () => fromQueryString,
  functionToWorker: () => functionToWorker,
  getEventTarget: () => getEventTarget,
  hover: () => hover,
  keyCode: () => keyCode,
  loadImage: () => loadImage,
  loadImageDataURI: () => loadImageDataURI,
  ...
  post: () => post,
  register: () => register,
  replaceSvgImports: () => replaceSvgImports,
  slide: () => slide,
  stopEvent: () => stopEvent,
  svgPointerPosn: () => svgPointerPosn,
  thread: () => thread,
...
var import_euclid3 = require("@mathigon/euclid");
...
var BrowserInstance = class {
  constructor() {
    ...
    const applyResizeThrottled = (0, import_core.throttle)(() => this.applyResize());
  ...
  setStorage(key, value) {
    ...
    const storage = (0, import_core.safeToJSON)(((_a2 = this.localStorage) == null ? void 0 : _a2.getItem(STORAGE_KEY)) || void 0, {});
    let path = storage;
  ...
  getStorage(key) {
    var _a2;
    let path = (0, import_core.safeToJSON)((_a2 = this.localStorage) == null ? void 0 : _a2.getItem(STORAGE_KEY), {});
    if (!key) return path;
    const keys = (key || "").split(".");
...
function animate(callback, duration) {
  ...
  const deferred = (0, import_core2.defer)();
  let lastTime = 0;
  let running = true;
  const cancel = () => {
    running = false;
...
function transition($el, properties, duration = 400, _delay = 0, easing = "ease-in-out") {
  ...
  Object.keys(properties).forEach((k) => {
    const p = properties[k];
    const k1 = (0, import_core2.toCamelCase)(k);
  ...
  if (to.height === "auto") {
    ...
    to.height = `${(0, import_core2.total)(children.map(($c) => $c.outerHeight))}px`;
  }
  let player;
  let cancelled = false;
  (0, import_core2.delay)(() => {
    if (cancelled) return;
...
var import_euclid = require("@mathigon/euclid");
...
function pointerPosition(e) {
  if (e.touches) {
    ...
    return new import_euclid.Point(touches[0].clientX, touches[0].clientY);
  } else {
    return new import_euclid.Point(e.clientX || 0, e.clientY || 0);
  }
}
...
function canvasPointerPosition(event, $canvas) {
  ...
  return new import_euclid.Point(x, y);
}
...
function makeTapEvent($el) {
  ...
  $el.on("pointerup", (e) => {
    ...
    if (import_euclid.Point.distance(start, end) < 6) $el.trigger("tap", e);
    start = void 0;
  });
...
function slide($el, fns) {
  ...
  function move(e) {
    ...
    e.preventDefault();
    const p = posn(e);
    if (import_euclid.Point.distance(p, lastPosn) < 0.5) return;
...
function hover($el, options) {
  ...
  $el.on("mouseout", () => {
    ...
    clearTimeout(timeout);
    timeout = (0, import_core3.delay)(exit2, options.exitDelay || options.delay);
  });
...
function bindEvent($el, event, fn, options) {
  if (event in customEvents) {
    ...
    const events = (0, import_core3.words)(aliases[event]);
...
function batch(callback) {
  ...
  if (batchDepth === 0) {
    for (const [callback2, state] of batchedCallbacks.entries()) {
      batchedCallbacks.delete(callback2);
      callback2(state);
    }
  }
}
...
var import_euclid2 = require("@mathigon/euclid");
...
function pathCommands(path) {
  ...
  for (const match of path.match(pathSegment) || []) {
    ...
    for (const [i, p] of (0, import_core4.chunk)(args, pathLength[uType]).entries()) {
      let points = [];
      let type = uType === "M" && i > 0 ? "L" : uType;
      let options = void 0;
      if (uType === "H") {
        type = "L";
        points = [new import_euclid2.Point(p[0], isAbsolute ? (lastPoint == null ? void 0 : lastPoint.y) || 0 : 0)];
      } else if (uType === "V") {
        type = "L";
        points = [new import_euclid2.Point(isAbsolute ? (lastPoint == null ? void 0 : lastPoint.x) || 0 : 0, p[0])];
      } else if (uType === "A") {
        type = "A";
        points = [new import_euclid2.Point(p[5], p[6])];
        options = p.slice(0, 5);
      } else if ("MLCSQT".includes(uType)) {
        points = (0, import_core4.chunk)(p, 2).map((q) => new import_euclid2.Point(q[0], q[1]));
      }
      if (!isAbsolute && lastPoint) points = points.map((p2) => p2.translate(lastPoint));
      lastPoint = (0, import_core4.last)(points);
...
function parsePath(d) {
  ...
  return commands.map((c) => (0, import_core4.last)(c.points)).filter((p) => !!p);
}
var COMMON_STYLES = [
  "font-family",
  "font-size",
...
var BaseView = class {
  ...
  addClass(className) {
    for (const c of (0, import_core5.words)(className)) this._el.classList.add(c);
  }
  removeClass(className) {
    for (const c of (0, import_core5.words)(className)) this._el.classList.remove(c);
  }
  hasClass(className) {
  ...
  get boundsRect() {
    ...
    return new import_euclid3.Rectangle(new import_euclid3.Point(bounds.x, bounds.y), bounds.width, bounds.height);
  }
  contains(point) {
  ...
  get isInViewport() {
    ...
    const bounds = this.bounds;
    return (0, import_fermat.isBetween)(bounds.top, -bounds.height, Browser.height);
  }
  get topLeftPosition() {
    const bounds = this.bounds;
    return new import_euclid3.Point(bounds.left, bounds.top);
  }
  get boxCenter() {
    const box = this.bounds;
    return new import_euclid3.Point(box.left + box.width / 2, box.top + box.height / 2);
  }
  get scrollWidth() {
  ...
  setTransform(posn, angle = 0, scale = 1) {
    let t = "";
    if (posn) t += `translate(${(0, import_fermat.roundTo)(posn.x, 0.1)}px,${(0, import_fermat.roundTo)(posn.y, 0.1)}px)`;
  ...
  translate(x, y) {
    this.setTransform(new import_euclid3.Point(x, y));
  }
  /**
  ...
  hasParent(...$p) {
    ...
    while (parent) {
      if ((0, import_core5.isOneOf)(parent, ...tests)) return true;
  ...
  on(events, callback, options) {
    for (const e of (0, import_core5.words)(events)) {
  ...
  onKey(keys, callback, options) {
    ...
    const keyNames = new Set((0, import_core5.words)(keys));
...
var SVGBaseView = class extends BaseView {
  ...
  setTransform(posn, angle = 0, scale = 1) {
    const t1 = posn ? `translate(${(0, import_fermat.roundTo)(posn.x, 0.1)} ${(0, import_fermat.roundTo)(posn.y, 0.1)})` : "";
    const t2 = (0, import_fermat.nearlyEquals)(angle, 0) ? "" : `rotate(${angle * 180 / Math.PI})`;
    const t3 = (0, import_fermat.nearlyEquals)(scale, 1) ? "" : `scale(${scale})`;
  ...
  getPointAtLength(d) {
    if (this._el instanceof SVGGeometryElement) {
      ...
      return new import_euclid3.Point(point.x, point.y);
    } else {
      return new import_euclid3.Point(0, 0);
    }
  }
  /**
  ...
  get center() {
    ...
    return new import_euclid3.Point(x, y);
  }
  ...
  draw(obj, options = {}) {
    ...
    this.setAttr("d", (0, import_euclid3.drawSVG)(obj, Object.assign(options, attributes)));
  }
};
...
var InputView2 = class extends HTMLBaseView {
  ...
  bindVariable(model, name) {
    ...
    if (this._el.type === "number") {
      ...
      this.change((v) => {
        if (v) model[name] = (0, import_fermat.clamp)(+v, min, max);
      });
    } else {
...
var CanvasView2 = class extends HTMLBaseView {
  ...
  draw(obj, options = {}) {
    this.ctx.save();
    (0, import_euclid3.drawCanvas)(this.ctx, obj, options);
    this.ctx.restore();
  }
  /** Clears this canvas. */
  clear() {
...
function $N(tag, attributes = {}, parent) {
  ...
  for (const [key, value] of Object.entries(attributes)) {
    ...
    if (key === "id") {
      ...
      el.setAttribute("d", (0, import_euclid3.drawSVG)(value));
    } else {
...
var loadImageDataURI = (0, import_core6.cache)((url) => __async(void 0, null, function* () {
...
function savePostData(url, data) {
  if (POST_DATA.has(url)) {
    (0, import_core6.deepExtend)(POST_DATA.get(url), data, (a, b) => (0, import_core6.unique)(a.concat(b)));
  } else {
...
var doDeferredPost = (0, import_core6.throttle)(sendPostData, 5e3);
...
// src/confetti.ts
var import_core7 = require("@mathigon/core");
var import_fermat2 = require("@mathigon/fermat");
var colours = (0, import_core7.loop)(["#cd0e66", "#0f82f2", "#22ab24", "#fd8c00"]);
var Particle = class {
  constructor(index) {
    this.index = index;
    this.color = colours();
    ...
    this.r = import_fermat2.Random.uniform(10, 30);
  }
  draw(ctx) {
    ctx.beginPath();
...
function confetti(duration = 2e3, maxParticles = 150, $parent = $body) {
  ...
  const particles = (0, import_core7.tabulate)((i) => new Particle(i), maxParticles);
...
// src/draggable.ts
var import_core8 = require("@mathigon/core");
var import_euclid4 = require("@mathigon/euclid");
var Draggable = class extends import_core8.EventTarget {
  constructor($el, options = {}) {
    super();
    this.$el = $el;
    this.startPos = new import_euclid4.Point(0, 0);
    this.position = new import_euclid4.Point(0, 0);
    this.disabled = false;
  ...
  updateBounds() {
    ...
    this.bounds = new import_euclid4.Bounds(0, width, 0, height);
  ...
  setPosition(x, y) {
    var _a2;
    let p = new import_euclid4.Point(this.options.moveX ? x : 0, this.options.moveY ? y : 0);
  ...
  resetPosition(duration = 250) {
    return __async(this, null, function* () {
      ...
      yield animate((p) => {
        const currentPos = import_euclid4.Point.interpolate(initial, this.startPos, p);
...
// src/router.ts
var import_core9 = require("@mathigon/core");
...
var Router = class extends import_core9.EventTarget {
  constructor() {
    super(...arguments);
    this.$viewport = $body;
    this.views = [];
...
// src/thread.ts
var import_core10 = require("@mathigon/core");
...
function thread(url, data, timeout = 5e3) {
  const deferred = (0, import_core10.defer)();
